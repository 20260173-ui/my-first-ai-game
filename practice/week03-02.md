...
#시행착오:처음 ai에게 삼각형의 움직임의 속도와 원의 크기가 커지는 속도가 예상과 달랐음
#따라서 ai에게 시간에 따라 원이 커지는 속도가 달라지게 해달라고 요청-->개발 성공. 삼각형의 속도는 직접 파일 내부에서 수정함
#배운점.:ai가 성능이 좋긴 하여도 세부적인 부분을 매우 자세히 설명하지 않으면 오류가 발생하거나 본인이 생각하지 못한 결과가 나올수 있다는것을 배움. 사용자가 원하는 결과를 위해선 최종 수정을 하거나 
#ai에게 더 자세한 세부사항 요청이 필요하다는걸 알게됨.
